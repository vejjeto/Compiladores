...
- **Backend Único:** Se ha eliminado el servidor independiente del frontend. Ahora el Backend (Node.js en el puerto 80 por defecto) sirve tanto la Interfaz Web como la API Rest y los WebSockets, ahorrando un 33% de memoria y simplificando el despliegue.
- **El Transmisor toma el control:** Toda la interfaz de conexión al carro y al Receptor Remoto se ha centralizado en el Transmisor. Él es quien ordena las conexiones.
...
- **Comunicación WebSocket Directa:** El Transmisor puede conectarse directamente a un Receptor remoto (ej. la PC de un compañero) vía WebSocket (`ws://<IP>/ws`) usando adaptadores dinámicos que traducen la data.
...
├── Backend/
│   ├── server.js                # Servidor unificado HTTP estático + API + WS (Puerto 80)
│   ├── config/
...
│   │   ├── adapters/
│   │   │   ├── peerAdapter.js   # Habilita la conexión saliente a Receptores
│   │   │   ├── wsCarAdapter.js  
│   │   │   └── wsServerAdapter.js # Servidor WebSocket API y Receptor entrante
│   │   ├── services/
...
└── simulador/
    └── esp32-simulator.js       # Simulador del carro local (Puerto 8081)
```
...
1. Abre `http://localhost` y selecciona **Receptor**.
...
### 3.3. Modo Multi-PC (Conexión Transmisor ➔ Receptor)

Para controlar el carro en la computadora de un compañero:
1. Pídele a tu compañero su IP y levanta tu sistema.
2. Abre tu **Transmisor** (`http://localhost`).
3. En **Conexión con Receptor**, escribe la ruta del websocket de su Receptor (ej: `ws://192.168.0.22/ws`).
4. Haz clic en **Conectar Receptor**.
5. Usa el botón "Conectar Carro" para ordenar a su Receptor enlazar el hardware, y envía tus programas.
6. Si deseas soltarle el control, pulsa **Desconectar** en el carro, lo que enviará remotamente el comando `M` (Liberar Control) al ESP32 de tu compañero de forma limpia y segura.
